...
# define RELEASE_WRITE_LOCK(){ \
    ...
    READ_TIMESTAMP(start_sus);\
      ...
      old_order_ts[local_thread_id].value=order_ts[local_thread_id].value;\
...
\
int temp1=-1;\
if(order_ts[local_thread_id].value!=0){\
  while(order_ts[local_thread_id].value-1 != temp1){\
    for(index=0; index < num_threads; index++){ \
    temp1=1;\
      if(index == local_thread_id) continue; \
      if(order_ts[local_thread_id].value-1 == order_ts[index].value || order_ts[local_thread_id].value-1 == old_order_ts[index].value){\
        printf("teste\n");\
	      temp1=1;\
      }\
      else{\
        /*printf("thread:%d my order-1:%d order=%d\n",local_thread_id,order_ts[local_thread_id].value-1,order_ts[index].value);*/\
      }\
	  } \
  if(temp1==1){\
    break;\
