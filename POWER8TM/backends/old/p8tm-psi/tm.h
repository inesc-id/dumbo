...
#  define TM_STARTUP(numThread, bId) my_tm_startup(numThread)
...
/*printf("first time: %d, second time: %d\n",total_first_time,total_second_time);*/ \
	fprintf(stderr, "%lu,%lu,%lu\n",global_numThread,wait_time,htm_conflict_aborts+htm_user_aborts+htm_capacity_aborts+htm_other_aborts+rot_conflict_aborts+rot_user_aborts+rot_capacity_aborts+rot_other_aborts);\
} \
...
// todo descomentar o for embaixo
# define RELEASE_READ_LOCK() { \
	...
	/*for(int index=0; index < num_threads; index++){ \
		...
		if(counters_snapshot[index] != 0){ \
			while(not_durable[index].value==1 && end_ts[index].value <= start_ts[local_thread_id].value){ \
				cpu_relax(); \
			} \
		} \
	}*/  \
	not_durable[local_thread_id].value=0; \
 	counters[local_thread_id].value+=7;\
...
# define USE_ROT(){ \
	...
	while(rot_budget > 0){ \
		...
		int state = counters[local_thread_id].value & 7; /*uses 3 bits, even value for inactive rots, odd value for active rots*/ \
		if (!state) \
		...
		/*if(rot_budget == 1){ \
			...
			READ_TIMESTAMP(start_time);*/ \
		while(mylogend-mylogpointer<MIN_SPACE_LOG){\
			load_fence();\
		}\
...
# define QUIESCENCE_CALL_ROTt1(){ \
	__attribute__((aligned(CACHE_LINE_SIZE))) static padded_scalar_t num_threads;\
	__attribute__((aligned(CACHE_LINE_SIZE))) static padded_scalar_t index;\
	__attribute__((aligned(CACHE_LINE_SIZE))) static padded_scalar_t state;\
	volatile __attribute__((aligned(CACHE_LINE_SIZE))) static padded_scalar_t temp; \
	__attribute__((aligned(CACHE_LINE_SIZE))) static padded_scalar_t start_wait_time; \
    __attribute__((aligned(CACHE_LINE_SIZE))) static padded_scalar_t end_wait_time; \
	num_threads.value= (long)global_numThread; \
/*	long counters_snapshot[80];*/ \
	for(index.value=0; index.value < 80; index.value++){ \
                if (index.value == num_threads.value) break; \
		if(index.value == local_thread_id) continue; \
		temp.value = counters[index.value].value; \
		state.value = temp.value & 7; \
		/*printf("state is %d,%d\n",state,temp); */\
		switch(state.value){ \
			case 7:\
	...
	for(index.value=0; index.value < num_threads.value; index.value++){ \
		if(index.value == local_thread_id) continue; \
		if(counters_snapshot[index.value] != 0){ \
			while(counters[index.value].value == counters_snapshot[index.value]){ \
				cpu_relax(); \
			} \
		} \
	} \
...
/*	long counters_snapshot[80];*/ \
	for(index=0; index < 80; index++){ \
		...
		if(index == local_thread_id) continue; \
		start_ts_snapshot[index].value=start_ts[index].value; \
	...
	not_durable[local_thread_id].value=1; \
	int teste=0;\
	for(index=0; index < global_numThread; index++){ \
		if(index == local_thread_id) continue; \
		if(counters_snapshot[index] != 0){ \
			/*commit_log(mylogpointer,order_ts[local_thread_id].value,mylogstart,mylogend);*/ \
			while(counters[index].value == counters_snapshot[index]  /*|| start_ts[index].value > start_ts_snapshot[index].value */){ \
				cpu_relax(); \
			} \
		} \
	} \
...
// orderTS[tid]=atomicInc(globalOrderTS); //obtain expected serialization order
// this is done as late as possible to reduce the chance that a thread gets a slot in the log and then aborts, preventing others to return from their commit request.

// after commit
// Wait while state[c]==not_durable && 
// (EndTS[c] <startTS[tid] || orderTS[c] < orderTS[tid])
...
# define RELEASE_WRITE_LOCK(){ \
	...
	if(local_exec_mode == 1){ \
		...
		for(int index=0; index < 1; index++){ \
			/*printf("------------\n");\
			printf("log          addr=%lx log val=%lx\n",mylogstart[teste],mylogstart[teste-1]);\
			printf("log_replayer addr=%lx log val=%lx\n",log_replayer_start_ptr[0][teste],log_replayer_start_ptr[0][teste+1]);*/\
			teste+=2;\
			if(counters_snapshot[index] != 0){ \
				while(not_durable[index].value==1 && order_ts[index].value <= order_ts[local_thread_id].value){ \
					cpu_relax(); \
				} \
			} \
		}  \
...
# define TM_BEGIN_EXT(b,ro) {  \
	...
	/* -------------------- */\
	start_ts[local_thread_id].value=rdtsc();\
	order_ts[local_thread_id].value=-1;\
	mylogpointer_snapshot=mylogpointer;\
	printf("pointer %p, snapshot %p\n",mylogpointer,mylogpointer_snapshot);\
	/* -------------------- */\
	if(ro){ \
		ACQUIRE_READ_LOCK(); \
	} \
