...
# define RELEASE_WRITE_LOCK(){ \
    if(local_exec_mode == 1){ \
            long b_aux=0; \
            __TM_suspend(); \
printf("%d, %d\n",stats_array[local_thread_id].read_commits, b_aux); \
            long end_time; \
            READ_TIMESTAMP(end_time); \
            if(local_thread_id == 0){ \
              tx_length[b_type.value].value = tx_length[b_type.value].value*0.5 + (end_time - start_time.value)*0.5; \
            } \
            /*		b_aux.value = 0;*/ \
...
printf("g %d\n",stats_array[local_thread_id].read_commits); \
            for(kill_index=0; kill_index < num_threads; kill_index++){ \
              if(kill_index != local_thread_id){ \
                if(counters[kill_index].value > end_time + tx_length[b_type.value].value*2){ \
