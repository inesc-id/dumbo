...
#define rmb()           __atomic_thread_fence(__ATOMIC_RELEASE)//asm volatile ("sync" ::: "memory")
#define rmb1()           __atomic_thread_fence(__ATOMIC_ACQUIRE)//asm volatile ("sync" ::: "memory")
...
# define RELEASE_WRITE_LOCK(){ \
	if(local_exec_mode == 1){ \
		...
		QUIESCENCE_CALL_ROT(); \
		__TM_end(); \
		printf("teste %d and %d\n",debug[0].value,debug[1].value); \
