...
mod_cb_add_on_abort(mod_cb_info_t *icb, void (*f)(void *arg), void *arg)
{
  if (unlikely(icb->abort_nb >= icb->abort_size)) {
    icb->abort_size *= 2;
    icb->abort = (mod_cb_entry_t*)xrealloc(icb->abort, sizeof(mod_cb_entry_t) * icb->abort_size);
  }
...
mod_cb_add_on_commit(mod_cb_info_t *icb, void (*f)(void *arg), void *arg)
{
  if (unlikely(icb->commit_nb >= icb->commit_size)) {
    icb->commit_size *= 2;
    icb->commit = (mod_cb_entry_t*)xrealloc(icb->commit, sizeof(mod_cb_entry_t) * icb->commit_size);
  }
...
static void mod_cb_on_thread_init(void *arg)
{
  ...
  icb->commit = (mod_cb_entry_t*)xmalloc(sizeof(mod_cb_entry_t) * icb->commit_size);
  icb->abort = (mod_cb_entry_t*)xmalloc(sizeof(mod_cb_entry_t) * icb->abort_size);
