...
@Algs = ("p8tm-psi-v2-fi-improved 0 20","p8tm-psi-v2-fi-improved-lc-hidden 0 20"#,"htm-sgl"
...
# Path to executables
$ExePath = "/home/miguel.figueiredo/projs/PersistentSiHTM/POWER8TM/benchmarks/bank/";
            ...
            for($w = 1; $w <= 1; $w+=0.25) {
                ...
                for ($t = 0; $t < $Trials; $t++) {
                    ...
                    if((grep /Time/ , @res)){
                        ...
                        @ressus = grep  /Total sus time: / , @res;
                        $ressus[0] =~ s/.*Total sus time: //;
                        $valsustime += int($ressus[0]);
                        
                        @resflush = grep  /Total flush time: / , @res;
                        $resflush[0] =~ s/.*Total flush time: //;
                        $valflushtime += int($resflush[0]);
                        
                        @reswt2 = grep  /Total wait2 time: / , @res;
                        $reswt2[0] =~ s/.*Total wait2 time: //;
                        $valwttime2 += int($reswt2[0]);
                        
                        @rescommits = grep  /Total commits: / , @res;
                        $rescommits[0] =~ s/.*Total commits: //;
                        $valcommits += int($rescommits[0]);
                        
                        @resreadcommits = grep  /Read commits: / , @res;
                        $resreadcommits[0] =~ s/.*Read commits: //;
                        $valreadcommits += int($resreadcommits[0]);
                        
                        @reshtmcommits = grep  /HTM commits: / , @res;
                        $reshtmcommits[0] =~ s/.*HTM commits: //;
                        $valhtmcommits += int($reshtmcommits[0]);
                        
                        @resrotcommits = grep  /ROT commits: / , @res;
                        $resrotcommits[0] =~ s/.*ROT commits: //;
                        $valrotcommits += int($resrotcommits[0]);
